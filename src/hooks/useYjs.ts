...
export function useYjs({ project }: { project: ClientProject }) {
  ...
  const updateLocalState = useCallback((doc: Y.Doc) => {
    ...
    yItineraries.forEach((yItinerary, itineraryId) => {
      const itinerary: Itinerary = {
        ...
        destinations: yItinerary
          .get('destinations')
          .toArray()
          .map((id: string) => yDestMap.get(id)?.toJSON()),
      }
    ...
    yOptimizedRoutes.forEach((yRoute, itineraryId) => {
      newOptimizedRoutes[itineraryId] = yRoute
        .toArray()
        .map((dest) => dest.toJSON())
    })
  ...
  const updateDayStartTime = useCallback(
    (itineraryId: string, newTime: string) => {
      if (!ydoc) return
  ...
  const setStartPlace = useCallback(
    (itineraryId: string, place: Destination) => {
      if (!ydoc) return
      const yItineraries = ydoc.getMap<Y.Map<any>>('itineraries')
      const yItinerary = yItineraries.get(itineraryId)
      if (yItinerary) {
  ...
  const addDestination = useCallback(
    (itineraryId: string, destination: Destination) => {
      if (!ydoc) return
      const yItineraries = ydoc.getMap<Y.Map<any>>('itineraries')
  ...
  const removeDestination = useCallback(
    (itineraryId: string, destinationId: string) => {
      if (!ydoc) return
      const yItineraries = ydoc.getMap<Y.Map<any>>('itineraries')
      const yDestMap = ydoc.getMap<Y.Map<any>>('destinations')
      const yItinerary = yItineraries.get(itineraryId)
      if (yItinerary) {
        const yDestinations = yItinerary.get('destinations') as Y.Array<string>
        const index = yDestinations
          .toArray()
          .findIndex((id) => id === destinationId)
        if (index !== -1) {
  ...
  const reorderDestinations = useCallback(
    (itineraryId: string, newOrder: string[]) => {
      if (!ydoc) return
      try {
        ydoc.transact(() => {
          const yItineraries = ydoc.getMap<Y.Map<any>>('itineraries')
          const yItinerary = yItineraries.get(itineraryId)
          if (yItinerary) {
            const yDestinations = yItinerary.get(
              'destinations',
            ) as Y.Array<string>
        ...
        console.error(
          `Failed to reorder destinations: ${err instanceof Error ? err.message : String(err)}`,
        )
      }
    },
    [ydoc],
  ...
  const updateDestinationDuration = useCallback(
    (destinationId: string, duration: number) => {
      if (!ydoc) return
      const yDestMap = ydoc.getMap<Y.Map<any>>('destinations')
  ...
  const updateDestinationDescription = useCallback(
    (destinationId: string, description: string) => {
      if (!ydoc) return
      const yDestMap = ydoc.getMap<Y.Map<any>>('destinations')
      const yDest = yDestMap.get(destinationId)
      if (yDest) {
  ...
  const setOptimizedRoute = useCallback(
    (itineraryId: string, route: Destination[]) => {
      if (!ydoc) return
